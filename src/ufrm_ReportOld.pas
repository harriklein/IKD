...
//  dm_Main.tb_Rental.Filter   := 'StartAt >= {d ' + (FormatDateTime('yyyy-mm-dd', FDate)) + '} AND StartAt < {d ' + (FormatDateTime('yyyy-mm-dd', FDate + 1)) + '}';
  dm_Main.tb_Rental.Filter   := 'Day = {d ' + (FormatDateTime('yyyy-mm-dd', FDate)) + '}';
  ...
  while not dm_Main.tb_Rental.Eof do
    begin
      ...
      LItem.Objects.FindDrawable('txtDateTime').Data :=                         dm_Main.tb_Rental.FieldByName('StartAt'          ).AsString;
      LItem.Objects.FindDrawable('txtType'    ).Data := PaymentMethod_EN_to_PT3(dm_Main.tb_Rental.FieldByName('StartPayMethod'       ).AsString);
      LItem.Objects.FindDrawable('txtTime'    ).Data :=                         dm_Main.tb_Rental.FieldByName('StartPayMinutes'      ).AsString + ' min.';
      LItem.Objects.FindDrawable('txtValue'   ).Data :=                 'R$ ' + dm_Main.tb_Rental.FieldByName('StartPayValue'        ).AsString + ',00';
...
begin
  ...
  lbl_Number.Text                   := dm_Main.tb_Rental.FieldByName('BoatNumber'              ).AsString;
  edt_RentFinishStartDate.Date      := dm_Main.tb_Rental.FieldByName('StartAt'             ).AsDateTime;
  edt_RentFinishStartHour.Date      := dm_Main.tb_Rental.FieldByName('StartAt'             ).AsDateTime;
  edt_RentFinishStartMinutes.Value  := dm_Main.tb_Rental.FieldByName('StartPayMinutes'  ).AsInteger;
  edt_RentFinishStartValue.Value    := dm_Main.tb_Rental.FieldByName('StartPayValue'    ).AsInteger;
...
//  edt_RentFinishCalcValue.Value     := dm_Main.tb_Rental.FieldByName('CalculedPaymentValue'    ).AsInteger;
  edt_RentFinishMinutes.Value       := dm_Main.tb_Rental.FieldByName('EndPayMinutes'          ).AsInteger;
  edt_RentFinishValue.Value         := dm_Main.tb_Rental.FieldByName('EndPayValue'            ).AsInteger;

  lbl_PaymentTypeValue.Text         := PaymentMethod_EN_to_PT(dm_Main.tb_Rental.FieldByName('StartPayMethod'    ).AsString);
